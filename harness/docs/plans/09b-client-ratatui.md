...
- [ ] ~~`orrery` runs a real turn end to end in a terminal, with working selection and scrollback for finished turns.~~ **Amended: not true, and not this plan's to make true.** The *client* does all of it — `tests/smoke.rs` drives the real loop end to end and asserts the settled turn is in scrollback and never redrawn — but the `orrery` **binary** cannot reach it: `orrery-cli` is plan 17's, and its bare `orrery` still exits 2 naming this plan file. Adding the dependency and the wiring is a change to `orrery-cli`, which this wave does not own. Unticked on purpose: the bullet as written stays false until 17 lands.
...
- **Not linked into the binary.** `orrery-cli` (plan 17) has no dependency on this crate and the bare `orrery` still exits 2. One `Cargo.toml` line and a `main` arm when 17 is worked.
