...
- ~~`orrery --ui ink` and a standalone run against `orrery serve` both render a real turn.~~ **Amended — not true, and not reachable from this plan.** `orrery serve` and `--ui` dispatch are plan 17's and both still exit 2, so neither invocation exists to run. What *is* true: `pnpm -C harness/clients/ink start` builds a bundle and runs it, a real turn renders over a real socket in `smoke.ink_renders_a_fixture_turn`, and the two failure modes a person hits first are sentences rather than stack traces (no endpoint → exit 2; unreachable kernel → exit 1 naming the endpoint). The remaining gap is the spawn in `orrery-cli`, which plan 17 owns.
