...
- [x] `harness/wit/orrery-extension.wit` — the world sketched in [`14`](14-wasm-wit.md), including the flat-arena surface. It does not have to be final; it has to exist so `wit-check` has a target and so the arena question is visible rather than deferred silently.
