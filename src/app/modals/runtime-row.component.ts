  ...
  template: `
    ...
    <div class="set-rt" [class.warn]="d?.status === 'error'">
      <div class="set-rt-top">
        ...
        @if (isOk() && !editing()) {
          <div class="set-rt-pathchip" [title]="d?.path">
            <app-icon name="terminal" size="sm" /><span class="pt">{{ d?.path }}</span>
          </div>
          <button type="button" class="set-rt-link" (click)="begin()">
            <app-icon name="rename" size="sm" />Change
          </button>
          @if (d?.source === 'manual') {
            <button type="button" class="set-rt-link" (click)="revert()" title="Re-run auto-detection">revert</button>
          }
        } @else {
          <span class="set-rt-spacer"></span>
        }
      </div>
        ...
        <div class="set-rt-edit">
          ...
          <div class="set-rt-field">
            <div class="set-rt-input">
              ...
              <input [value]="draft()" [placeholder]="'/path/to/' + toolId()" spellcheck="false"
