  ...
  template: `
    ...
    @if (n.isDir) {
      <div>
        ...
        @if (open()) {
          @if (n.children) {
            ...
            <div [style.padding-left.px]="pad() + 30" style="font-size:var(--fs-xs);color:var(--ink-4);padding-top:var(--sp-1);padding-bottom:var(--sp-1)">loading…</div>
          }
        }
      </div>
    } @else {
      <div
        ...
        style="display:flex;align-items:center;gap:var(--sp-3);padding:var(--sp-1) var(--sp-4);border-radius:5px"
      >
        <app-icon name="file" size="sm" [px]="12" color="var(--ink-4)" />
        <span [style.color]="n.ignored ? 'var(--ink-4)' : 'var(--ink-3)'" [style.opacity]="n.ignored ? 0.7 : 1" style="font-size:var(--fs-sm);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ n.name }}</span>
      </div>
    }
  `,
})
