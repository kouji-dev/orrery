    ...
    <div style="position:relative;padding-left:var(--sp-9)">
      ...
      @for (c of commits(); track $index) {
        ...
        >
          ...
          <div
            [title]="c.msg"
            style="color:var(--ink);line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden"
          >{{ c.msg }}</div>
          <div class="tnum" style="display:flex;align-items:center;gap:var(--sp-3);margin-top:var(--sp-1);font-size:var(--fs-meta);color:var(--ink-4)">
            @if (!compact()) { <span [style.color]="color(c.agent)">{{ ag ? ag.name : c.agent }}</span> }
            <kj-badge>{{ c.sha }}</kj-badge>
