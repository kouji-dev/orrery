...
/** JSON shape of the whole service state — persisted in the workspace document. */
export interface ScrollSnapshot {
  plain: Record<string, number>;
  view: Record<string, monacoApi.editor.ICodeEditorViewState>;
  diff: Record<string, monacoApi.editor.IDiffEditorViewState>;
}
 ...
 *
 * The maps are plain (not signals) — `rev` is bumped on every mutation so the
 * WorkspaceStore's persistence effect can watch for changes and serialize via
 * {@link snapshot}; {@link hydrate} restores a persisted snapshot at boot.
 */
...
export class ScrollStateService {
  ...
  /** Mutation counter — the persistence trigger. */
  ...
  saveView(agentId: string, path: string, state: monacoApi.editor.ICodeEditorViewState | null): void {
    if (!state) return;
    this.view.set(k(agentId, path), state);
    this.rev.update((n) => n + 1);
  }
  ...
  saveDiffView(agentId: string, path: string, state: monacoApi.editor.IDiffEditorViewState | null): void {
    if (!state) return;
    this.diff.set(k(agentId, path), state);
    this.rev.update((n) => n + 1);
  }
  ...
  /** The whole state as JSON, for the persisted workspace document. */
  snapshot(): ScrollSnapshot {
    return {
      plain: Object.fromEntries(this.plain),
      view: Object.fromEntries(this.view),
      diff: Object.fromEntries(this.diff),
    };
  }

  /** Restore a persisted snapshot (boot). Replaces current state; does NOT bump
   *  `rev` — hydration must not trigger a write of what was just read. */
  hydrate(s: Partial<ScrollSnapshot> | null | undefined): void {
    if (!s) return;
    this.plain.clear();
    this.view.clear();
    this.diff.clear();
    for (const [key, v] of Object.entries(s.plain ?? {})) this.plain.set(key, v);
    for (const [key, v] of Object.entries(s.view ?? {})) this.view.set(key, v);
    for (const [key, v] of Object.entries(s.diff ?? {})) this.diff.set(key, v);
  }
}
