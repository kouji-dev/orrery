// Builds the branded updater stub (ade/src-tauri/updater-stub) in release mode
// and stages it where tauri.conf.json's bundle.resources expects it
// (ade/src-tauri/binaries/orrery-updater.exe). The cargo target dir is the
// workspace root's target/, one level above ade/. Runs in beforeBuildCommand
// AND beforeDevCommand — tauri-build validates the resource path on EVERY
// cargo build of the app, so a fresh clone must run this once before
// `cargo build` or `cargo test` in ade/src-tauri will fail with
...
copyFileSync(
  join(root, "..", "target", "release", "orrery-updater.exe"),
