...
test("menu items highlight on hover; danger gets its own tint; Escape closes", async ({ page }) => {
  ...
  const alpha = menu.getByRole("button", { name: "Alpha" });
  const idle = await alpha.evaluate((el) => getComputedStyle(el).backgroundColor);
  await alpha.hover();
  const hovered = await alpha.evaluate((el) => getComputedStyle(el).backgroundColor);
  ...
  const del = menu.getByRole("button", { name: "Delete" });
  await del.hover();
