...
function DiffOrBlame({ ag, path, diff, addDel, onOpenCommit }) {
  ...
  return <FileDiff path={path} diff={diff} headerRight={right} />;
}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0, background: "var(--panel-2)" }}>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "232px 1fr", minHeight: 0 }}>
        ...
        {selPath ? <DiffOrBlame key={selPath} ag={ag} path={selPath} diff={diff} addDel={f && <AddDel add={f.add} del={f.del} gap={6} />} />
